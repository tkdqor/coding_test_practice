...
- 📖 **Contents**
  ...
  - [리스트 마지막 요소 제외하고 for문 설정하기](#리스트-마지막-요소-제외하고-for문-설정하기)
...
- 아래와 같이 **해시**를 사용해서도 List Comprehension으로 key와 value값을 정해줄 수 있다.
```python
sizes = [1,2,3,4,5]
a = {size:0 for size in sizes}
print(a)
...
- **이제 개념적인 차이점을 살펴보자. 만약 hello 클래스의 num 속성에 접근하려면 어떻게 해야할까? 우선 객체로 접근하는 것이 아니기 때문에 self.num를 사용할 순 없다. 즉, 클래스 내부에 self.num = 10 이렇게 정의되어 있지 않기 때문이다. num = 10과 같은 num 변수는 객체로 접근할 수 없는 변수이다.** 억지로 사용하고 싶다면 @staticmethod는 다음과 같이 해야 한다.
...
- 📖 **Contents**
  ...
  - [완전탐색](#완전탐색)
    ...
    - [2개의 변수에서 한 변수에는 큰 수를 몰아주고 다른 변수에는 작은 수를 몰아주기](#2개의-변수에서-한-변수에는-큰-수를-몰아주고-다른-변수에는-작은-수를-몰아주기)
  - [정렬](#정렬)
    - [로또 번호 일치 개수를 리스트의 인덱스라고 생각하기](#로또-번호-일치-개수를-리스트의-인덱스라고-생각하기)
    - [for문으로 2차원 배열의 리스트 1개씩 뽑기](#for문으로-2차원-배열의-리스트-1개씩-뽑기)
    - [for문으로 2차원 배열의 리스트 요소들을 바로 뽑기](#for문으로-2차원-배열의-리스트-요소들을-바로-뽑기)
  ...
  - [ETC](#etc)
    ...
    - [2차원 리스트에서 행과 열 그리고 대각선 합 구하기](#2차원-리스트에서-행과-열-그리고-대각선-합-구하기) 
...
### 2개의 변수에서 한 변수에는 큰 수를 몰아주고 다른 변수에는 작은 수를 몰아주기
- 2개의 정수로 이루어져있는 리스트로 만들어진 2차원리스트에서 for문으로 배열 1개씩 뽑을 때, 둘 중 큰 수와 작은 수를 나눠서 각각의 변수로 설정하기
  - [관련 문제](https://github.com/tkdqor/coding_test_practice/blob/master/programmers/%EC%99%84%EC%A0%84%ED%83%90%EC%83%89/%EC%B5%9C%EC%86%8C%EC%A7%81%EC%82%AC%EA%B0%81%ED%98%95.py)
...
### for문으로 2차원 배열의 리스트 요소들을 바로 뽑기
- 2차원 배열에서 for문으로 리스트 1개의 요소들을 바로 뽑을 수 있다.
```python
...
### 2차원 리스트에서 행과 열 그리고 대각선 합 구하기
- 2차원 리스트를 격자판이라고 생각할 때, 2중 for문으로 행의 합과 열의 합 구하기
  - 0행의 합과 0열의 합 구하고, 1행의 합과 1열의 합 구하고 ... 반복
```python
a = [[10,13,10,12,15], ...]
for i in range(n):
...
- 마찬가지로 2차원 리스트를 격자판이라고 생각할 때, 대각선 2개의 합 구하기
  - for문으로 인덱스를 뽑고, 왼쪽 끝부터 오른쪽 아래까지의 대각선은 a[i][i] / 오른쪽 끝부터 왼쪽 아래까지의 대각선은 a[i][n-i-1] 이렇게 표현 
```python
sum1 = sum2 = 0
for i in range(n):
